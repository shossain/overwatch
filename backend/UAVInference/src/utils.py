...
def detect_state_change(previous_context: list[dict], current_context: list[dict]) -> str:
...
def analyze_frames(previous_frame: list[dict], current_frame: list[dict]):
    ...
    added_objects = [curr_objects[name] for name in curr_objects if name not in prev_objects]
    removed_objects = [prev_objects[name] for name in prev_objects if name not in curr_objects]
    overlapping_objects = [curr_objects[name] for name in curr_objects if name in prev_objects]

    # Describe added and removed objects
    for obj in added_objects:
        changes.append(f"New {obj['prompt']} '{obj['id']}' detected at centroid {obj['centroid']}")
    ...
    for obj in overlapping_objects:
        ...
        if obj["prompt"] == "smoke":
            ...
            if obj["centroid"] == matching_prev_obj["centroid"]:
                patterns.append(f"{obj['prompt']} '{obj['id']}' remained stationary at centroid {obj['centroid']}")
            else:
                displacement = [round(curr - prev, 2) for curr, prev in zip(obj["centroid"], matching_prev_obj["centroid"])]
                patterns.append(f"{obj['prompt']} '{obj['id']}' moved by displacement {displacement}")
...
def analyze_scene(previous_frame: list[dict: any], current_frame: list[dict: any]):
    ...
    for current_obj in current_frame:
        ...
        for prev_obj in previous_frame:
            if current_obj["id"] == prev_obj["id"]:
                ...
                if (current_obj["centroid"] == prev_obj["centroid"] and
                        current_obj["size"] == prev_obj["size"]):
    ...
    if new_objects:
        ...
        for obj in new_objects:
            descriptions.append(f"- id {obj['id']} of type {obj['prompt']} at centroid {obj['centroid']} with size {obj['size']}")
    if removed_objects:
        descriptions.append("\nRemoved objects:")
        for obj in removed_objects:
            descriptions.append(f"- id {obj['id']} of type {obj['prompt']} at centroid {obj['centroid']} with size {obj['size']}")
    if unchanged_objects:
        ...
        for obj in unchanged_objects:
            descriptions.append(f"- id {obj['id']} of type {obj['prompt']} at centroid {obj['centroid']} with radius {obj['size']}")
        ...
        if obj["prompt"] == "vehicle":
            ...
            if nearby_smoke:
                ...
                for smoke in nearby_smoke:
                    descriptions.append(f"- Smoke {smoke['name']} at centroid {smoke['centroid']} with radius {smoke['radius']}")
